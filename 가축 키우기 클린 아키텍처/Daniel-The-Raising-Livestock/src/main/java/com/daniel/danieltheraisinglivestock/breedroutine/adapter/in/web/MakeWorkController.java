...
public class MakeWorkController {
    ...
    private final MakeWorkUseCase makeWorkUseCase;

    @PostMapping("/breed/work/{adminBreedId}/{dailyBreedId}/{health}")
    void makeWork(
            @PathVariable Long adminBreedId,
            @PathVariable Long dailyBreedId,
            @PathVariable int health) {

        BreedCommand makeWorkCommand = new BreedCommand(
                new Breed.BreedId(adminBreedId),
                new Breed.BreedId(dailyBreedId),
                LiveStock.of(health)
        );

        makeWorkUseCase.makeWorkLiveStock(makeWorkCommand);
    }
}
